...
export default function ProDashboard() {
  ...
  async function fetchJobs() {
    ...
    setLoading(true);
    try {
      let res = await fetch("/api/pro/jobs?status=open", { credentials: "include" });
      if (!res.ok) res = await fetch("/api/jobs/open", { credentials: "include" });
  ...
  async function act(jobId, action) {
    try {
      let res = await fetch(`/api/pro/jobs/${jobId}/${action}`, { method: "POST", credentials: "include" });
      if (!res.ok) res = await fetch(`/api/jobs/${jobId}`, { method: "PATCH", credentials: "include", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ action }) });
  ...
  async function toggleActive() {
    setActive(a => !a);
    try {
      let res = await fetch("/api/pro/active", { method: "POST", credentials: "include", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ active: !active }) });
