...
const iconMap = { "Plumbing":"🔧","Electrical":"⚡","Carpentry":"🔨","Painting":"🎨","HVAC":"❄️","Roofing":"🏠","House Cleaning":"🧽","Junk Removal":"🗑️","Landscaping":"🌿" };
export default function HomePage(){
  ...
  return (<>
    ...
    <div className="container-xl">
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          ...
          <div className="card p-6">
            <ul className="grid grid-cols-2 gap-3 text-sm text-slate-300">
              {["Background Checked pros","Real-time SMS updates","AI Assistant guidance","Secure Payments"].map(b=>(
                <li key={b} className="flex items-center gap-2"><span>✅</span><span>{b}</span></li>
              ))}
            </ul>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {services.map(s=>(
            <div key={s} className="card p-5">
              ...
              <div className="mt-4"><Link to={`/services/${s.toLowerCase().replace(/\s+/g,'-')}`} className="btn btn-primary">Explore</Link></div>
            </div>
          ))}
        </div>
      </section>
      <section className="py-8">
        ...
        <div className="grid md:grid-cols-3 gap-4 mt-4">
          {[{city:'Miami',slug:'miami-fl'},{city:'New York',slug:'new-york-ny'},{city:'Los Angeles',slug:'los-angeles-ca'},{city:'Chicago',slug:'chicago-il'},{city:'Houston',slug:'houston-tx'},{city:'Phoenix',slug:'phoenix-az'}].map(({city, slug})=>(
            <div key={slug} className="card p-4">
              <h3 className="font-semibold">{city}</h3>
              <div className="mt-2 flex flex-wrap gap-2 text-sm">
                {['plumbing','electrical','hvac'].map(svc=>(<Link key={svc} to={`/services/${svc}/${slug}`} className="pill">{svc} in {city}</Link>))}
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="py-6 grid md:grid-cols-4 gap-4">
        {[{icon:'✅', t:'Background Checked', d:'Verified professionals you can trust'},{icon:'📱', t:'Real-time SMS', d:'Instant notifications for new jobs'},{icon:'🤖', t:'AI Assistant', d:'Smart tools to grow your business'},{icon:'💳', t:'Secure Payments', d:'Fast, secure payment processing'}].map(b=>(
          <div key={b.t} className="card p-5"><div className="text-2xl">{b.icon}</div><div className="mt-1 font-semibold">{b.t}</div><div className="text-sm text-slate-400">{b.d}</div></div>
        ))}
      </section>
    ...
    <Modal open={open} onClose={()=>setOpen(false)} title="Request a Free Quote"><LeadForm/></Modal>
  </>);
}
function LeadForm(){
  ...
  const submit = async (e)=>{ e.preventDefault();
    try{ await fetch(`${api}/api/leads`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(form)}); alert('Thanks! We will text you shortly.'); }
  ...
  return (<form onSubmit={submit} className="space-y-3">
    ...
    <div><label className="block text-sm text-slate-300">Service</label>
    ...
    <div><label className="block text-sm text-slate-300">City</label><input className="w-full rounded-xl border border-white/10 bg-slate-900 px-3 py-2" value={form.city} onChange={e=>setForm({...form, city:e.target.value})} required/></div>
    <div className="flex items-center gap-2"><input id="home-sms" type="checkbox" className="h-4 w-4" required/><label htmlFor="home-sms" className="text-xs text-slate-400">I agree to receive SMS related to quotes, scheduling, and service updates. Reply STOP to unsubscribe.</label></div>
