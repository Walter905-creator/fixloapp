...
export default function ServiceRequestForm({ presetService = "", presetCity = "", onSubmitted }) {
  ...
  async function submit() {
    ...
    try {
      let photoUrl = null;
      try { if (file) photoUrl = await uploadFile(file, "fixlo/leads"); } catch (e) { console.warn("Photo upload failed:", e.message); }
      const payload = { ...form, source: "website", photoUrl };
      ...
      {ok ? (
        ...
        <form onSubmit={(e) => (e.preventDefault(), submit())}>
          ...
          <div style={{marginTop: 12}}>
            <label style={label}>Optional photo</label>
