...
export default function ProSignupForm({ onSubmitted }) {
  ...
  try {
    ...
    // Preferred: one-shot checkout endpoint that creates the pro in DB and returns Stripe url
    const res = await fetch("/api/pros/checkout", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify(payload)
    });
    // If the proxy/endpoint is misconfigured, this could return HTML — try text first
    ...
    if (ctype.includes("application/json")) {
      ...
      try { data = JSON.parse(t); } catch { throw new Error(`Unexpected response (status ${res.status}).`); }
    }
    ...
    // Fallback path: if no url came back, show success and keep old behavior
    setOk(true);
    onSubmitted?.();
  } catch (e) {
    // Legacy fallbacks — try legacy endpoints to at least save the Pro
    try {
      let profilePhotoUrl = null;
      try { if (file) profilePhotoUrl = await uploadFile(file, "fixlo/pros"); } catch (e) { console.warn("Photo upload failed:", e.message); }
      const payload = { ...form, role: "pro", profilePhotoUrl };
      ...
      let r = await postTo("/api/pros/signup");
      if (!r.ok) r = await postTo("/api/pros");
      if (!r.ok) r = await postTo("/api/signup/pro");
      if (r.ok) {
        setOk(true);
        onSubmitted?.();
        setError("");
      ...
      {ok ? (
        ...
        <form onSubmit={(e) => (e.preventDefault(), submit())}>
          ...
          <div style={{marginTop: 12}}>
            <label style={label}>Optional profile photo</label>
